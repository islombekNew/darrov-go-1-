...
export function LoginScreen({ navigation }: any) {
  ...
  const onSend = async () => {
    ...
    try {
      ...
      navigation.navigate('LoginOtp', { phone: full });
    } catch (e: any) {
      setErr(e.message);
    } finally {
      setLoading(false);
        ...
        <ScrollView contentContainerStyle={{ padding: S.lg, paddingBottom: S.xxl }} keyboardShouldPersistTaps="handled">
          ...
          <View style={[ss.row, { backgroundColor: T.bg2, borderColor: ok ? C.gn : err ? C.rd : T.bd }]}>
            ...
            <TextInput
              ...
              value={phone}
              onChangeText={t => { setPhone(t.replace(/\D/g, '').slice(0, 9)); setErr(''); }}
          ...
          <TouchableOpacity style={[ss.btn, { backgroundColor: ok ? C.p : T.bg3 }]} onPress={onSend} disabled={!ok || loading} activeOpacity={0.87}>
            {loading ? <ActivityIndicator color="#fff" /> : <Text style={[ss.bTxt, { color: ok ? '#fff' : T.t4 }]}>Kod yuborish</Text>}
          </TouchableOpacity>
        </ScrollView>
...
export function LoginOtpScreen({ navigation, route }: any) {
  ...
  const onChange = (txt: string, i: number) => {
    const d = txt.replace(/\D/g, '').slice(-1);
    const next = [...digits]; next[i] = d; setDigits(next);
    if (d && i < 5) refs.current[i + 1]?.focus();
    if (i === 5 && d) verify(next.join(''));
  };
  ...
  const verify = async (code: string) => {
    ...
    try {
      const result = await api.post<{ token: string; user: any; isNew: boolean }>('/auth/verify-otp', { phone, code });
      setAuth(mapUser(result.user), result.token);
      if (result.isNew) navigation.navigate('RegRole', { phone });
    } catch (e: any) {
      setErr(e.message);
      setDigits(['', '', '', '', '', '']);
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: T.bg }} edges={['top']}>
      <ScrollView contentContainerStyle={{ padding: S.lg, paddingBottom: S.xxl }}>
        ...
        <Text style={[ss.sub, { color: T.t3 }]}>+{phone} raqamiga 6 xonali kod yuborildi</Text>
        <View style={{ flexDirection: 'row', justifyContent: 'center', gap: rs(8, 10), marginVertical: S.lg }}>
          {digits.map((d, i) => (
            <TextInput key={i} ref={el => { refs.current[i] = el; }}
              style={{ width: rs(46, 56), height: rs(60, 73), borderWidth: 2.5, borderColor: d ? (all ? C.gn : C.p) : T.bd, borderRadius: R.md, fontSize: rs(26, 32), fontWeight: '900', color: T.t1, backgroundColor: T.bg2, textAlign: 'center' }}
              value={d} onChangeText={t => onChange(t, i)} onKeyPress={e => onKey(e, i)} keyboardType="numeric" maxLength={1} selectTextOnFocus
            />
          ))}
        </View>
        {!!err && <View style={{ backgroundColor: '#fff1f1', borderRadius: R.md, padding: S.md, marginBottom: S.md, borderWidth: 1, borderColor: C.rd }}>
          <Text style={{ color: C.rd, fontWeight: '700', fontSize: F.sm, textAlign: 'center' }}>{err}</Text>
        </View>}
        <Text style={{ textAlign: 'center', fontSize: F.sm, color: T.t3, fontWeight: '700', marginBottom: S.md }}>
          {timer > 0 ? `Kod ${timer} soniyada eskiradi` : 'Kod eskirdi'}
        </Text>
        {timer === 0 && (
          <TouchableOpacity onPress={resend} style={{ alignItems: 'center', marginBottom: S.md }}>
            <Text style={{ color: C.p, fontWeight: '800', fontSize: F.md }}>Qayta yuborish</Text>
          </TouchableOpacity>
        )}
...
export function RegInfoScreen({ navigation, route }: any) {
  ...
  const onNext = async () => {
    ...
    try {
      const full = normalizePhone(phone);
      await api.post('/auth/send-otp', { phone: full });
      navigation.navigate('RegOtp', { role, name: name.trim(), phone: full });
    } catch (e: any) {
...
export function RegOtpScreen({ navigation, route }: any) {
  ...
  const onChange = (txt: string, i: number) => {
    ...
    const next = [...digits]; next[i] = d; setDigits(next);
    if (d && i < 5) refs.current[i + 1]?.focus();
    if (i === 5 && d) navigation.navigate('RegRegion', { phone, name, role, code: next.join('') });
  };
  const onKey = (e: any, i: number) => {
    if (e.nativeEvent.key === 'Backspace' && !digits[i] && i > 0) refs.current[i - 1]?.focus();
  };
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: T.bg }} edges={['top']}>
      <ScrollView contentContainerStyle={{ padding: S.lg, paddingBottom: S.xxl }}>
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'center', gap: rs(8, 10), marginVertical: S.lg }}>
          {digits.map((d, i) => (
            <TextInput key={i} ref={el => { refs.current[i] = el; }}
              style={{ width: rs(46, 56), height: rs(60, 73), borderWidth: 2.5, borderColor: d ? C.p : T.bd, borderRadius: R.md, fontSize: rs(26, 32), fontWeight: '900', color: T.t1, backgroundColor: T.bg2, textAlign: 'center' }}
              value={d} onChangeText={t => onChange(t, i)} onKeyPress={e => onKey(e, i)} keyboardType="numeric" maxLength={1} selectTextOnFocus
            />
          ))}
        </View>
        {!!err && <Text style={{ color: C.rd, fontWeight: '700', textAlign: 'center', marginBottom: S.sm }}>{err}</Text>}
        <TouchableOpacity style={[ss.btn, { backgroundColor: all ? C.p : T.bg3 }]} onPress={() => navigation.navigate('RegRegion', { phone, name, role, code: digits.join('') })} disabled={!all || loading} activeOpacity={0.87}>
          {loading ? <ActivityIndicator color="#fff" /> : <Text style={[ss.bTxt, { color: all ? '#fff' : T.t4 }]}>Davom etish</Text>}
        </TouchableOpacity>
      </ScrollView>
    </SafeAreaView>
  );
...
export function RegRegionScreen({ navigation, route }: any) {
  const { T } = useThemeStore();
  const { phone, name, role, code } = route.params;
  ...
  const onDone = async () => {
    ...
    try {
      // 1. OTP tasdiqlash
      const verResult = await api.post<{ token: string; user: any; isNew: boolean }>('/auth/verify-otp', { phone, code });
      const region = REGIONS.find(r => r.id === sel);

      // 2. Ro'yxatdan o'tishni yakunlash
      const regResult = await api.post<{ token: string; user: any }>('/auth/complete-register', {
        phone,
        name,
        role,
        regionName: region?.name ?? 'Toshkent',
      }, verResult.token);

      setAuth(mapUser(regResult.user), regResult.token);
    } catch (e: any) {
      Alert.alert('Xato', e.message);
    } finally {
      setLoading(false);
