...
import { getRestaurantById } from '../../constants/data';
...
export function MenuScreen({ navigation, route }: any) {
  ...
  const rest = getRestaurantById(restId);
  ...
  const cats = Array.from(new Set(rest.menu.map(m => m.category)));
    ...
    if (res === 'conflict') {
      Alert.alert(
        ...
        [
          ...
          { text: 'Ha, tozalash', onPress: () => { cart.clearCart(); cart.addItem({ menuItemId:item.id, name:item.name, price:item.price }, rest.id, rest.name); } },
        ]
      );
    }
  };
    ...
    <SafeAreaView style={[ms.safe, { backgroundColor: T.bg }]} edges={['top']}>
      <View style={[ms.hdr, { backgroundColor: T.hdrBg, borderBottomColor: T.bd }]}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={[ms.hdrSub, { color: T.t3 }]}>⭐ {rest.rating} · {rest.deliveryTime}</Text>
        </View>
      </View>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: cartCnt > 0 ? rs(90, 110) : S.xl }}>
        ...
        <View style={[ms.banner, { backgroundColor: isDark ? T.bg3 : C.plt }]}>
          <Text style={{ fontSize: rs(60, 72) }}>{rest.emoji}</Text>
        </View>

        {cats.map(catId => {
          const items = rest.menu.filter(m => m.category === catId);
          const catName = catId === 'milliy' ? 'Milliy taomlar'
            : catId === 'fastfood' ? 'Fast Food'
            : catId === 'soglom' ? "Sog'lom taomlar"
            : catId === 'pizza' ? 'Pizza'
            : catId === 'sushi' ? 'Sushi'
            : catId === 'ichimlik' ? 'Ichimliklar' : catId;
          return (
            <View key={catId}>
              <Text style={[ms.catTitle, { color: T.t1 }]}>{catName}</Text>
              {items.map(item => {
                const isHl = highlight && item.name.toLowerCase().includes(highlight.toLowerCase());
                return (
                  <View key={item.id} style={[
                    ...
                    <View style={[ms.itemImg, { backgroundColor: isDark ? T.bg3 : C.plt }]}>
                      <Text style={{ fontSize: rs(30, 36) }}>{item.emoji}</Text>
                    </View>
                    <View style={{ flex: 1 }}>
                      <Text style={[ms.itemName, { color: T.t1 }]}>{item.name}</Text>
                      <Text style={[ms.itemDesc, { color: T.t3 }]} numberOfLines={2}>{item.desc}</Text>
      ...
      {cartCnt > 0 && (
        <TouchableOpacity style={[ms.cartFab, { backgroundColor: C.p }]} onPress={() => navigation.navigate('Cart')} activeOpacity={0.9}>
          <View style={ms.cartBadge}><Text style={ms.cartBadgeTxt}>{cartCnt}</Text></View>
          <Text style={ms.cartFabLbl}>Savatga o'tish</Text>
          <Text style={ms.cartFabTotal}>{fmtPrice(cart.total())} →</Text>
        </TouchableOpacity>
      )}
    </SafeAreaView>
  );
...
export function CartScreen({ navigation }: any) {
  ...
  const placeOrder = () => {
    Alert.alert(
      'Buyurtma berish',
      `Jami: ${fmtPrice(cart.total())}\n${cart.earnCoins()} coin olasiz!`,
      [
        { text: 'Bekor', style: 'cancel' },
        { text: 'Tasdiqlash', onPress: () => {
          navigation.navigate('Tracking', { fromCart: true });
        }},
      ]
    );
  };
  ...
  return (
    <SafeAreaView style={[ms.safe, { backgroundColor: T.bg }]} edges={['top']}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ padding: S.lg, paddingBottom: rs(180, 220) }}>
        {items.map(item => (
      ...
      <View style={[s2.bottom, { backgroundColor: T.hdrBg, borderTopColor: T.bd }]}>
        ...
        <TouchableOpacity style={[s2.orderBtn, { backgroundColor: C.p }]} onPress={placeOrder} activeOpacity={0.88}>
...
const s2 = StyleSheet.create({
  ...
  coinInfo: { flexDirection: 'row', alignItems: 'center', gap: S.sm, borderWidth: 1.5, borderRadius: R.md, padding: S.md, marginTop: S.md },
  ...
  orderBtn: { borderRadius: R.lg, paddingVertical: rs(15, 19), alignItems: 'center', marginTop: S.md },
